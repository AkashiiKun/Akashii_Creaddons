package io.github.akashiikun.creaddon.content.liquidfuel.core;
...
import io.github.akashiikun.creaddon.mixin.BlazeBurnerAccessor;
import io.github.akashiikun.creaddon.util.Triplet;
...
public class BurnerStomachHandler {
    ...
    public static void tick(SmartBlockEntity entity) {
        ...
        int newBurnTime = burnerAccessor.creaddon$getRemainingBurnTime() + burnerProperty.getFirst();
        if (newBurnTime > BlazeBurnerBlockEntity.MAX_HEAT_CAPACITY) return;

        if (fluidSuperHeats)
            burnerAccessor.creaddon$invokeSetBlockHeat(BlazeBurnerBlock.HeatLevel.SEETHING);
        else
            burnerAccessor.creaddon$invokeSetBlockHeat(BlazeBurnerBlock.HeatLevel.FADING);

        burnerAccessor.creaddon$setRemainingBurnTime(newBurnTime);
